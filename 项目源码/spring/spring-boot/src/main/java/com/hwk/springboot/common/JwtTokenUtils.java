...
public class JwtTokenUtils {
    ...
    @PostConstruct
    public void setUpAdminService() {
        staticAdminService = adminService;
    }
    
    //生成token
    public static String genToken(String adminId,String password) {
        return JWT.create().withAudience(adminId) // 将userid 保存到 token 里面，作为载荷
                .withExpiresAt(DateUtil.offsetHour(new Date(), 2)) // 2小时后token过期
                .sign(Algorithm.HMAC256(password));// 以password 作为 token 的密钥
    }
    //获取当前登录的用户信息
    public static Admin getCurrentAdmin() {
        ...
        try {
            ...
            if(StrUtil.isBlank(token)){
                log.error("获取当前登录用户失败，token :{}",token);
                return null;
            }
            //解析token，获取用户id
            ...
            log.error("获取当前登录用户失败，token={}", token, e);
            return null;
        }
    }
}
