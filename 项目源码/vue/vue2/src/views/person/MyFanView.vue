...
export default {
  ...
  methods: {
    ...
    follow(fandata) {
      ...
      this.FollowForm.followid = fandata.id;
      this.isfollow = this.Myfollow.includes(fandata.id);
      console.log(fandata.myfan);
      const handleResponse = (res) => {
        if (res.code === "0") {
          ...
          if (this.isfollow) {
            ...
            this.$set(fandata, 'myfan', fandata.myfan - 1);
          } else {
            this.admin.myfollow += 1;
            this.$set(fandata, 'myfan', fandata.myfan + 1);
          }
    ...
    updateUser(){
      ...
      request.post("/admin",this.fan).then(res => {
        if(res.code === "0") {
        }
        else{
          this.$message({
